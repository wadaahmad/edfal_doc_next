...
export default function MenuFinder() {
    ...
    return (
        <div className="form-group pt-3 px-2 my-1" id="__BVID__17">
            <div><div role="group" className="input-group input-group-sm">
                <input id="filter-input" type="search" placeholder="Type to Search" 
                autoComplete="off" value={filterText} 
                onChange={(e) => setFilterText(e.target.value)} onKeyUp={search} 
                className="form-control" />
                <div className="input-group-append"></div>
            </div>
                {filterText.length > 0 &&
                    <div className="bg-white px-2 py-3 border border-secondary" 
                    style={{ position: 'absolute', width: '500px', zIndex: 2147483647, display: 'block' }}>
                        {isFound
                            ? <ul className="pl-0">
                                {
